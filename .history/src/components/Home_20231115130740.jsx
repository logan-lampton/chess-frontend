    ...
    <div className='h-screen bg-gray-100'>
      <div className='grid grid-cols-2 gap-20 content-around w-screen flex items-center px-12 mt-20'>
        {clubs.map((club) => (
          ...
          >
            <Link to={`/clubs/${club.id}`} state={{ club }} className='w-2/3'>
              <div className='bg-gray-900 text-white font-bold py-2 px-4 border hover:bg-gray-700'>
                <h2>{club.club_name}</h2>
              </div>
            </Link>
            <button className='w-1/3 bg-red-600 hover:bg-red-400 text-white font-bold py-2 px-4 border bg-gray-900 rounded mr-4'>
              Delete Club
            </button>
          </div>
        ))}
